...
public class LookCompalintConterller {
    ...
    @ResponseBody
    @ApiOperation(value = "投诉设为属实")


    public String updateCom(Model model, Integer id , HttpServletRequest req) throws IOException, ParseException {

        Complaint complaint = complaintService.selectByPrimaryKey(id);
        if(complaint.getSign().equals(2))
        {
            System.out.println(id);
            System.out.println("------------------------");
            complaint.setSign(3);
            complaintService.updateByPrimaryKey(complaint);
        }else if(complaint.getSign().equals(3)){
            System.out.println(id);
            complaint.setSign(2);
            complaintService.updateByPrimaryKey(complaint);
        }else {
        ...
        @RequestMapping(value = "/lookCom",method = RequestMethod.GET)

    @ApiOperation(value = "查看投书")
    public String lookCom(Model model, Integer id , HttpServletRequest req) throws IOException, ParseException {
        ...
        if(eduUser==null)
        {
            model.addAttribute("登陆超时,重新登陆");
            return "/main/500";
        }
        System.out.println(eduUser);
        Education education=educationService.findByAreaId(eduUser.getAreaId()+"");
        SimpleDateFormat simpleDateFormat= new SimpleDateFormat("yyyy-MM-dd");
        String ss=   simpleDateFormat.format(complaint.getCreateDate());

        complaint.setFlag2(ss);
        complaint.setCname(education.getEname());
