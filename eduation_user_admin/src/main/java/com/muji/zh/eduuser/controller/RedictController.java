...
public class RedictController {
    ...
    @ApiOperation(value = "用户定向", httpMethod = "GET")
//    @ResponseBody
    public String redict( HttpServletRequest request, HttpServletResponse response, HttpSession session, Model model) throws IOException, ParseException {


        EduUser eduUser = (EduUser) request.getSession().getAttribute("eduUser");
        if (eduUser==null)
        {
            String s="用户名或密码错误";
